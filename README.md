...
|---|---|---|
| `GET` | `/` | API root |
| `GET` | `/api/v1/cities/{id}` | Get city detail by ID (full record + dr5hn enrichment + postcodes + translations + data quality) |
| `GET` | `/api/v1/cities/{id}/postcodes` | All postcodes for a city (paginated, state-scoped) |
| `GET` | `/api/v1/cities/{id}/translations` | All 19 translations for a city (ar, ja, zh-CN, etc.) |
| `GET` | `/api/v1/cities/{id}/translations/{lang}` | City name in a single language |
| `GET` | `/api/v1/cities/{id}/airports` | Airports serving a city (data pending) |
| `GET` | `/api/v1/cities/search` | Search cities by name (FTS5 + ranking + state filter + cross-language) |
| `GET` | `/api/v1/translations/search` | Find cities by translated name (e.g. ?q=東京&lang=ja) |
| `GET` | `/api/v1/postcodes/search` | Find cities by postal code (e.g. ?code=32501&country=US) |
| `GET` | `/api/v1/airports/near` | Find airports near a point (lat/lon/radius) |
| `GET` | `/api/v1/health` | Health check (DB stats + latency) |
| `GET` | `/api/v1/status` | API status and service info |
| `GET` | `/api/v1/data-quality` | Data quality audit (confidence, sources, migrations) |
| `GET` | `/api/v1/data-quality/issues` | List data quality issues (filterable by type) |
...
_16 endpoints across 7 route files._
